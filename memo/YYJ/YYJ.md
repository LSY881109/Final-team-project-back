...
### **4단계: 서비스 통합**
- [ ] `AIAnalysisService` (또는 상위 서비스)에서 최종 DTO 조립 로직 구현 (담당 팀원과 협업)
  - `FoodReferenceRepository`와 `YoutubeApiService`를 호출하여 각각 `FoodReference` Entity와 `List<YoutubeRecipeDTO>`를 가져옵니다.
  - `AIAnalysisService` 내에서 `ModelMapper`를 사용하여 `FoodReference` Entity를 `NutritionData` DTO로 변환하는 로직을 구현합니다.
